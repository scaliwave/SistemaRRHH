    ...
    if (window.location.pathname === "/administrador/listar-usuarios") {
      ...
      useEffect(() => {
        const updateUsuario = async (id, usuario) => {
          try {
            const response = await axios.put(`http://localhost:3000/api/usuarios/${id}`, usuario, {
              headers: {
                ...
                <form onSubmit={(e) => editarUsuario(e, usuario.id)} key={usuario.id}>
                  ...
                  <div className="usuario">
                    <div className="row">
                      ...
                      <div className="col"><h4>Usuario: <br /> <CampoDeTexto campoTexto={usuario.usuario} opcionEditar={usuario.editMode} onGuardar={(nuevoTexto) => guardarTexto(usuario.id, nuevoTexto, 'usuario')}></CampoDeTexto></h4></div>
                      <div className="col"><h4>Cédula: {usuario.id}</h4></div>
                    </div>
                    <div className="row">
                      <div className="col"><h4><CampoDeTexto campoTexto={usuario.cargo} opcionEditar={usuario.editMode} onGuardar={(nuevoTexto) => guardarTexto(usuario.id, nuevoTexto, 'cargo')}></CampoDeTexto></h4></div>
                      <div className="col"><h4>Contraseña: *******</h4></div>
                      <div className="col"><h4><Btn1 name={usuario.editMode ? 'Guardar' : 'Editar usuario'} size={[width, 30]} /></h4></div>
                    </div>
                  </div>
                </form>
              )))}
